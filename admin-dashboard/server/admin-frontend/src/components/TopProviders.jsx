...
const TopProviders = ({ data }) => {
  const [view, setView] = useState('table'); // view: 'table', 'bar', 'line', 'podium'
  if (!data || data.length === 0) return null;

  const podium = data.slice(0, 3); // top 3 only for podium
  ...
  return (
    <div className="mt-12">
      ...
      <div className="bg-blue-600 text-white px-6 py-3 rounded-t-md shadow-md flex justify-between items-center">
        <div>
          <h3 className="text-lg font-semibold">Top Providers by Activity Count</h3>
          <p className="text-sm text-blue-100">Top 5 Providers 🏆</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          {['table', 'bar', 'line', 'podium'].map(type => (
            <button
              key={type}
              className={`px-3 py-1 rounded ${view === type ? 'bg-white text-blue-600' : 'bg-blue-500'}`}
              ...
              <tbody>
                {data.map((p, idx) => (
                  <tr key={idx} className="border-t hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2">{p.activity_count}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex justify-center items-end gap-6 mt-6">
            {podium.map((provider, index) => (
              ...
              >
                ...
                <div className={`font-semibold ${podiumStyles[index].text}`}>{provider?.provider_name}</div>
                <div className={`text-sm ${podiumStyles[index].text}`}>{provider?.activity_count} activities</div>
              </div>
            ))}
          </div>
        )}
