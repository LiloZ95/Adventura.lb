...
const TopCategoriesByRevenue = ({ data }) => {
    ...
    <div className="mt-12">
      ...
      <div className="bg-blue-600 text-white px-6 py-3 rounded-t-md shadow-md flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-blue-100">Top 3 Categories 💸</p>
        </div>
              ...
              <tbody>
                {data.map((cat, idx) => (
                  <tr key={idx} className="border-t hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2">${parseFloat(cat.total_revenue).toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex justify-center items-end gap-6 mt-6">
            {podium.map((cat, index) => (
              ...
              >
                ...
                <div className={`text-sm ${podiumStyles[index].text}`}>${parseFloat(cat?.total_revenue).toFixed(2)}</div>
              </div>
            ))}
          </div>
        )}
...
export default TopCategoriesByRevenue;
